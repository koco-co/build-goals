...
- 由 `audit-agent-setup` 调用时遵循上层的审查或修复授权；由 `build-plugin` 统筹时沿用其任务范围和恢复条件，不重复确认。
...
3. 读取 `workflows/§03-design.md`、`rules/architecture.md`、`rules/frontmatter.md`、`rules/quality-standard.md` 和 `rules/platform-compatibility.md`，形成最小设计；只有用户要求先评审设计，或存在会改变结构、行为、权限或验收的未决选择时才等待确认。
4. 授权范围明确后读取 `workflows/§04-implementation.md`，实现该范围，复用确定性工具并保持单一规范源。
...
- 用户明确要求创建、升级或重构时已授权对应 Skill 文件的本地编辑；不自动 commit、push、发布、安装或更新 Plugin，并保留用户未提交修改。
