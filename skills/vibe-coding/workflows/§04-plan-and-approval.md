...
全局文档必须标记“文档状态：已确认”，记录项目路线、旧项目参考政策、允许参考范围、需求快照或迁移基线、方案比较、共同架构和跨域验证。功能域文档记录本域边界、需求或 Finding 映射、组件与依赖、接口和数据契约、验证策略与回退；存在 UI 时引用需求包中的可见状态或迁移 Finding，只保存最终确认的技术状态流，不复制界面草图和产品状态机。
