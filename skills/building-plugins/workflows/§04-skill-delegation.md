...
Plugin 中的每个 Skill 必须满足 `building-skills` 的结构、命名、渐进式读取、机械校验和语义验收要求。
...
- 平台支持受控委派：明确调用 `building-skills` 并传递已确认范围；
- 平台不支持嵌套调用：输出可直接交给 `building-skills` 的交接内容，等待用户显式调用；
- 不在本工作流中复制 `building-skills` 的完整实现。
