...
description: 仅由用户显式调用的 Agent Skill 构建与升级工作流。用于从零设计、实现和验证通用 Skill 或项目级定制 Skill，也用于审查并重构已有低质量 Skill；当前适配 Claude Code 与 Codex。不要因普通提示词润色、文档编写或一般编码请求自动进入本 Skill。
