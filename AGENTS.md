...
- `.claude-plugin/` 与 `.codex-plugin/`：Claude Code 和 Codex 的 Plugin Manifest；`.agents/plugins/marketplace.json` 是 Codex Marketplace 配置。
...
- 实现和验证结束后，主动询问用户是否执行当前任务需要的 commit、push、Claude Code Plugin 更新和 Codex Plugin 更新；允许只授权部分操作。
