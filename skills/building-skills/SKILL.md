...
description: 仅由用户显式调用的 Agent Skill 构建与升级工作流。用于从零设计、实现和验证通用 Skill 或项目级定制 Skill，也用于审查并重构已有低质量 Skill；适配 Claude Code、Codex 及开放 Agent Skills 规范。不要因普通提示词润色、文档编写或一般编码请求自动进入本 Skill。
compatibility: 遵循开放 Agent Skills 规范；平台专属行为按需适配 Claude Code 与 Codex。运行内置机械校验时需要 Python 3.9 或更高版本。
metadata:
  author: koco-co
  version: "1.0.0"
---
...
把一个明确的 Skill 需求转化为结构精简、行为可靠、能够真实运行和验证，并适配目标 Agent 平台的完整实现。
...
- 用户只要求润色提示词、编写普通文档或处理与 Skill 无关的任务时，不进入本 Skill；说明边界后结束或转交更合适的能力。
- 无法确认是否为显式调用时，不推断触发；先要求用户明确是否调用 `building-skills`。
...
1. 查明事实
   ...
   - 调研当前平台规范以及同类实现；用户明确免除外部竞品调研时，仍需核对目标环境和平台契约。
   - 不向用户询问能够从环境、代码或文档中自行查明的事实。
   - 完成条件：形成可追溯的现状与约束摘要，标明已确认事实、推断和未知项。
...
3. 提出设计并等待确认
   ...
   - 设计必须解释为何保留每个目录；不得为了形式完整创建空目录或占位文件。
...
4. 执行
   ...
   - 保持单一规范源；平台差异集中在适配文件或安装过程，不复制两套核心工作流。
...
- 保护用户代码、凭据、隐私数据和仓库权限；不得在示例、日志或报告中泄露秘密。
- 用户确认设计前保持只读；实施确认不等于授权提交、推送、发布、删除或修改仓库权限。
- 遇到删除、覆盖稳定接口、迁移目录、改变触发行为、执行外部副作用或提高权限时，必须取得对应范围的明确授权。
- 不制造问题、不重复询问、不虚构调研结论、不声称执行了未运行的命令或未完成的平台验证。
- 可机械判断的要求必须由 CLI、脚本、测试或结构化校验承担；模型判断只处理语义和场景层面的问题。
...
- 需要独立审阅者或 Subagent 时，读取 `prompts/independent-reviewer.prompt.md`；不涉及多 Agent 时不要读取。
