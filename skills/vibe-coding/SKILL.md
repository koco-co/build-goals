...
- 项目需要新增或升级 Agent Skill、Plugin、README 时，分别转交 `build-skill`、`build-plugin`、`build-readme`；平台不能受控委派时，生成完整交接提示，由用户继续调用对应 Skill。
...
4. 构建可验证脚手架
   ...
   - 脚手架必须先通过自身静态检查，再形成一个可独立回滚的本地提交。
...
5. 调起 Agent 开发团队并按功能交付
   ...
   - 每个 Agent 只领取一个可独立验证的功能切片，先写失败测试，再实现最小代码，随后重构并运行该切片要求的全部质量检查。
