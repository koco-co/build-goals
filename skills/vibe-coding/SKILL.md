...
- 项目需要新增或升级 Agent Skill、Plugin、README 时，分别转交 `build-skill`、`build-plugin`、`build-readme`；平台不能受控委派时，输出完整交接内容并要求用户显式调用对应 Skill。
- 只要求澄清想法时转交 `shape-idea`；只要求整理会话交接时转交 `handoff`。
- 普通代码修改、单点 Bug 修复、单个文档编写或没有完整项目交付目标的任务，不进入本 Skill。
- 无法确认是否为显式调用时，不推断触发；要求用户明确调用 `vibe-coding`。
...
1. 建立只读基线并选择路线
   ...
   - 完成条件：目标路线、输入产物、现状基线、现有用户改动、可用工具和阻塞项均已明确。

2. 调研并提出架构方案
   - PRD 驱动分支完整读取 `workflows/§02-greenfield-architecture.md`。
   - 已有项目分支完整读取 `workflows/§03-migration-audit.md`。
   - 调研前读取 `rules/modern-engineering.md`、`rules/repository-audit.md` 和 `rules/orchestration-contract.md`。
...
4. 构建可验证脚手架
   ...
   - 脚手架必须先通过自身机械检查，再形成一个可独立回滚的本地提交。
   - 根据依赖图为无依赖且文件所有权不重叠的任务创建独立 worktree；有依赖的任务保持串行。
   - 完成条件：干净环境可以按文档安装、启动并运行第一组测试，任务与 worktree 映射已经冻结。
...
5. 调起 Agent 开发团队并按功能交付
   ...
   - 每个 Agent 只领取一个可独立验证的功能切片，先写失败测试，再实现最小代码，随后重构并运行该切片全部质量门禁。
   - 每个功能单元通过验收后创建一次本地 Git commit；提交必须包含任务与需求追踪信息，不混入其他任务。
   - 并行任务通过 worktree 隔离；集成由唯一的 Integration Manager 按依赖顺序执行，并在每次集成后重跑受影响检查。
   - 平台没有 Subagent 或 worktree 能力时，以角色隔离的串行方式执行，并在交付中明确降级情况。
   - 完成条件：所有确认任务均已实现、集成并关联到唯一提交，没有遗漏需求或未说明的范围漂移。

6. 完成全链路验证
   - 完整读取 `workflows/§07-validation.md`。
   - 读取 `rules/acceptance-standard.md`，使用 `checklists/final-acceptance.md` 完成语义验收。
   - 运行格式化、Lint、类型检查、构建、单元测试、集成测试和端到端测试；有前端或可视界面时，再执行视觉、响应式、无障碍和组件交互验证。
   - 使用真实可复现的正常测试数据验证平台流程，并清理调试数据、脏数据、占位文案、PRD 原文和用户口述文案。
   - 检查安全密钥、环境变量、权限、依赖、日志、错误处理、文档与运行入口。
   - 运行 `scripts/validate_delivery.py` 的 delivery 阶段严格校验；失败时修复并重跑。
   - 完成条件：所有适用质量门禁通过，未验证项和阻塞项有准确证据，不把静态存在描述为真实功能已通过。
...
7. 交付并停止
   ...
   - README 与项目事实不一致时转交 `build-readme`；不在本 Skill 中复制 README 构建流程。
   - 完成条件：用户能够定位全部产物、提交和验证证据，并能明确判断已验证、未验证与阻塞项。
...
- 使用“已验证、未验证、阻塞”区分机械检查、真实用户流程、视觉验证和外部环境限制。
...
- 搭建脚手架和 worktrees 时，完整读取 `workflows/§05-scaffold-and-worktrees.md`。
