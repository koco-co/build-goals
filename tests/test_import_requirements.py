...
IMPORTER = REPO_ROOT / "skills" / "vibe-coding" / "scripts" / "import_requirements.py"
VALIDATOR = REPO_ROOT / "skills" / "build-prd" / "scripts" / "validate_prd.py"


class ImportRequirementsTests(unittest.TestCase):
    def run_importer(self, source: Path, target: Path, *extra: str) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            [sys.executable, str(IMPORTER), str(source), str(target), *extra],
            cwd=REPO_ROOT,
            check=False,
            text=True,
            capture_output=True,
    ...
    def test_write_imports_valid_snapshot_and_records_portable_source(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            source = write_requirement_package(root / "source")
            target = root / "target"
            ...
            manifest = json.loads((imported / "需求包清单.yaml").read_text(encoding="utf-8"))
            self.assertEqual(manifest["import"]["source_project"], "test-fixture/task-product")
            self.assertEqual(manifest["import"]["source_revision"], "fixture")
            self.assertNotIn(str(source.resolve()), json.dumps(manifest["import"], ensure_ascii=False))
    ...
    def test_same_snapshot_ignores_local_import_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            source = write_requirement_package(root / "source")
            target = root / "target"
            self.assertEqual(self.run_importer(source, target, "--write").returncode, 0)
    ...
    def test_existing_snapshot_requires_explicit_replace_and_reports_impact(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            target = root / "target"
            self.assertEqual(self.run_importer(source_v1, target, "--write").returncode, 0)
            ...
            replaced = self.run_importer(source_v2, target, "--write", "--replace")
            self.assertEqual(replaced.returncode, 0, replaced.stdout + replaced.stderr)
            self.assertIn(
                "记录并确认",
                (target / "docs" / "产品需求" / "功能域" / "任务管理.md").read_text(encoding="utf-8"),
            )
    ...
    def test_manifest_only_change_requires_explicit_replace(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            replaced = self.run_importer(source_v2, target, "--write", "--replace")
            self.assertEqual(replaced.returncode, 0, replaced.stdout + replaced.stderr)
            imported = json.loads((target / "docs" / "产品需求" / "需求包清单.yaml").read_text(encoding="utf-8"))
