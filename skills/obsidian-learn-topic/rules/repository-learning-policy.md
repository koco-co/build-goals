...
- 外层路线固定为：`01-项目概述`、`02-运行与测试基线`、`03-架构与模块地图`、`04-核心调用链`、`05-测试与质量体系`、`06-Issue与PR考古`、`07-最小修复实践`、`08-深入与拓展`、`09-复习与贡献准备`、`10-学习记录`、`99-assets`。只允许在知识正文阶段内细分连续编号子目录；学习证据只进入 `10-学习记录`。
- 整体地图只建立一次；正式学习只选择一条核心切片，且它必须能从入口追到行为与测试，避免按文件逐个漫游。
...
完整课程计划与 Commit 基线持久化在 `01-项目概述/§01-学习路线图.md`。每个仓库单元仍遵守“单项可验收成果、唯一知识归属、显式前置、正文与学习记录分离”。
...
- 没有合格 checkout 时，只推荐 Vault 外隔离位置；用户提供并确认精确绝对路径后，才使用 `scripts/repository_cli.py` 在该位置准备 detached Commit 基线。模型不自行选择或创建外部根路径。
...
`graduation_status` 允许 `pending-evidence`、`blocked`、`passed`：
...
- 未满足任一条件保持 `pending-evidence`，路线和相关单元不得声称仓库学习毕业。

未经独立授权，不 commit、push、fork、创建 Issue/PR，也不发送外部消息。

Patch 摘要、用户解释、测试结果与毕业判定写入对应学习记录；知识正文只保存稳定的调用链、机制、修复思路和可复用边界。

`09-复习与贡献准备` 只整理维护信号、贡献规范和后续方向。贡献准备度不能替代真实 Patch 与测试的学习毕业证据，任何上游动作仍需独立授权。
...
`upstream_status` 的完整词汇固定为 `unchanged`、`fixed-baseline`、`changed`、`blocked`、`archived`；`graduation_status` 的完整词汇固定为 `pending-evidence`、`blocked`、`passed`。路线合同、Frontmatter、可见基线、仓库执行器和证据文件必须使用同一组词汇。

确认迁移到新基线后，只能用 `curriculum-map` 的 compare-and-swap replace 同步新版课程合同与仓库元数据。`repository_provider`、canonical `owner/repo` 和 URL 始终与旧锚点一致；default branch、target ref、Commit、许可证、核验日期、学习范围、核心切片和上游状态只在这次路线图事务中更新。普通正文或学习记录写入必须继续完全匹配 Vault 当前锚点，不能夹带基线迁移。
