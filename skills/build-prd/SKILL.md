...
- 当前会话已通过 `shape-idea` 确认同一主题时，复用确认结论；规定的调研已经完成则直接写入 PRD，否则只补充缺失调研及其引出的新决策。
...
2. 确认所有产品决策
   - 若当前会话已经通过 `shape-idea` 得到确认结论，且调研充分，直接沿用确认结论，不重复提问。否则按照决策顺序，一次只询问一个会改变 PRD 的问题；每题都给出基于事实与调研的推荐答案，直到用户明确确认所有产品决策。
...
4. 验证并交付
   ...
   - 完成条件：静态检查和内容清单均通过，目标项目没有发生 PRD 之外的写入。
...
- 使用本 Skill 只授权创建或更新目标 PRD；不得修改目标项目代码、配置、数据、凭据、任务系统或外部服务状态。
...
- 最终 PRD 不得包含 TODO、TBD、假设、开放问题、待确认内容、未解决问题或模糊占位文案。
