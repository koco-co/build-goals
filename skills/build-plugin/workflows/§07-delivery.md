...
先判断本次是独立调用，还是由 `health-check` 等上层流程受控调用。两种模式都必须完成交付报告，不得因受控调用省略 Plugin 与 Skill 的验证结果。

### 受控调用

不重复询问 commit、push、Marketplace、Release、发布、部署或本地 Plugin 更新，也不自行执行。向上层总控返回：
...
尚未由上层创建提交时明确写“由上层待生成并回填”。发现新的产品、架构、公开接口、权限或范围决策时，返回上层暂停处理。
