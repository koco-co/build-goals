"""Requirement, finding, behavior-sample, task, and commit traceability validation."""
...
SAMPLE_RE = re.compile(r"\b(?:SAMPLE-[A-Z0-9-]+|[A-Z][A-Z0-9-]*SAMPLE[A-Z0-9-]*)\b")
...
def validate_tasks(
    ...
    if len(set(numbers)) != len(numbers) or sorted(numbers) != expected:
        add_issue(
            issues,
            "error",
            "TASK_SEQUENCE",
            path,
            f"任务编号必须在整个项目从 001 连续且唯一；实际为 {numbers}。文件或功能域顺序不影响判定。",
            root,
        )
    for task in tasks:
        for label in TASK_LABELS:
            ...
            if not value or value in {"-", "N/A", "待生成", "待开始"}:
                add_issue(issues, "error", "TASK_FIELD", path, f"{task.task_id} 缺少有效字段“{label}”。", root)
        first_evidence = _first_validation_evidence(task)
        if not first_evidence or first_evidence in {"-", "N/A", "待生成", "待开始"}:
            add_issue(
                issues,
                "error",
                "TASK_FIELD",
        ...
        if status == "已完成":
            ...
            if not worktree:
                add_issue(issues, "error", "TASK_WORKTREE", path, f"{task.task_id} 缺少 Worktree 或 N/A 记录。", root)
            integration_status = _field(task.body, "集成状态")
            if not integration_status or "已集成" not in integration_status:
                add_issue(issues, "error", "TASK_INTEGRATION_STATUS", path, f"{task.task_id} 已完成但缺少已验证的集成状态。", root)
...
def validate_traceability(
    ...
    if mode in {"greenfield", "continuation"}:
        identifiers = sorted(
            set(FUNCTION_RE.findall(source_text))
            | set(AC_RE.findall(source_text))
            | set(SAMPLE_RE.findall(source_text))
        )
        code = "PRD_TRACEABILITY"
        label = "PRD 需求/验收/行为样例"
    else:
