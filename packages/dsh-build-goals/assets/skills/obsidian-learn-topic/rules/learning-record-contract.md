...
- 提问消息只包含进度状态、场景、问题与按需提示；提示逐步揭示，首次尝试前不直接给答案。
- 回答后先用三句以内说明结论、证据缺口和下一步，再写入记录。

### 提问消息格式

面向用户发送检查点或复习题时，进度状态保持一行简短说明；场景与问题分别使用二级标题：
...
`场景`、`问题`不得写成 `场景：……`、`问题：……` 这类正文标签，也不把本轮问题命名为“任务”。学习记录中的同一结构从属于 `## 当前检查点`，因此使用 `### 场景`、`### 问题`，并按需生成完整的 `### 提示` 小节。
...
- 每次复习把问题、结果、证据来源和下一次日期追加到同一学习记录。
