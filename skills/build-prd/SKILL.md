...
- 当前会话已通过 `clarify-goal` 确认同一主题时，复用确认结论；规定的调研已经完成则直接写入 PRD，否则只补充缺失调研及其引出的新决策。
...
2. 确认所有产品决策
   - 若当前会话已经通过 `clarify-goal` 得到确认结论，且调研充分，直接沿用确认结论，不重复提问。否则按照决策顺序，一次只询问一个会改变 PRD 的问题；每题都给出基于事实与调研的推荐答案，直到用户明确确认所有产品决策。
