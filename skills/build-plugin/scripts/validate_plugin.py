...
from validate_plugin_core import *  # noqa: F403,E402
...
def _validate_claude_marketplace(plugin_root: Path, report: Report) -> None:
    ...
    if schema is not None and not _nonempty(schema):
        _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_SCHEMA", path, "$schema 存在时必须是非空字符串。", plugin_root)

    name = data.get("name")
    if not isinstance(name, str) or _core.NAME_RE.fullmatch(name) is None:
        _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_NAME", path, "Marketplace name 必须是 kebab-case。", plugin_root)

    owner = data.get("owner")
    if not isinstance(owner, dict) or not _nonempty(owner.get("name")):
        _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_OWNER", path, "Marketplace owner 必须包含非空 name。", plugin_root)
    ...
    if not isinstance(plugins, list) or not plugins:
        _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_PLUGINS", path, "Marketplace plugins 必须是非空数组。", plugin_root)
        return

    matched_manifest = False
    for index, item in enumerate(plugins):
        if not isinstance(item, dict):
            _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_ENTRY", path, f"plugins[{index}] 必须是对象。", plugin_root)
            continue
        entry_name = item.get("name")
        if not isinstance(entry_name, str) or _core.NAME_RE.fullmatch(entry_name) is None:
            _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_PLUGIN_NAME", path, f"plugins[{index}].name 必须是 kebab-case。", plugin_root)
        ...
        if isinstance(source, str) and source.startswith("./"):
            ...
            _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_SOURCE", path, f"plugins[{index}].source 必须是字符串或对象。", plugin_root)

        for field in ("homepage", "repository"):
            if field in item and not _nonempty(item.get(field)):
                _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_URL", path, f"plugins[{index}].{field} 存在时必须是非空字符串。", plugin_root)

    if manifest_name and not matched_manifest:
        _core.add_issue(report.issues, "error", "CLAUDE_MARKETPLACE_MANIFEST_MISSING", path, f"Marketplace 没有声明当前 Claude Plugin {manifest_name!r}。", plugin_root)
