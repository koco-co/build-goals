...
7. 交付、请求发布确认并停止
   ...
   - 实现和验证完成后，只列出真实适用的 commit、push、Marketplace、Claude Code Plugin 与 Codex Plugin 动作，并主动询问用户是否执行。
   - 明确用户可以全部授权或只授权其中部分动作；不得在用户回答前执行任何交付动作。
   - 当前 Plugin 交付后停止，不自行开始下一个 Plugin 或其他平台适配。
   - 完成条件：用户能够定位全部变更、复现验证、判断是否验收，并对适用的交付动作作出选择。
...
- 实现和验证完成后必须主动询问适用的交付动作；不得在用户回答前执行，也不得列出目标仓库或平台并不存在的动作。
